						   ...
						   unsigned int *prediv, unsigned int *fbdiv)
{
	...
	unsigned int _prediv, best_prediv = 1;
	unsigned int _fbdiv, best_fbdiv = 1;
					      ...
					      unsigned long *prate)
{
	...
	return inno_video_phy_pll_round_rate(inno, *prate, rate, NULL, NULL);
}
					   ...
					   unsigned long parent_rate)
{
	...
	inno->pll.rate = inno_video_phy_pll_round_rate(inno, parent_rate, rate,
					     &inno->pll.prediv, &inno->pll.fbdiv);

	dev_info(inno->dev, "fin=%lu, rate=%lu, fout=%lu, prediv=%u, fbdiv=%u\n",
		parent_rate, rate, inno->pll.rate, inno->pll.prediv, inno->pll.fbdiv);
